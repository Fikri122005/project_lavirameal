...
export default function HomeScreenSiswa({ navigation }) {
  ...
  return (
    <View style={styles.container}>
          ...
          {stats.riwayat.length > 0 ? stats.riwayat.map((item, idx) => (
             <View key={idx} style={styles.activityCard}>
                ...
                <View style={{ flex: 1, marginLeft: 12 }}>
                   ...
                   <Text style={styles.activityTime}>{new Date(item.created_at).toLocaleString('id-ID')}</Text>
                </View>
                <Text style={styles.activityAmount}>-{formatIDR(item.amount)}</Text>
             </View>
          )) : (
      ...
      >
        <View style={styles.navMainInner}>
...
const styles = StyleSheet.create({
  ...
  walletValue: { fontSize: 22, fontWeight: '900', color: BLUE_PRIMARY, marginTop: 4 },
