...
try {
    ...
    // UPDATE SALDO SPPG (MENGURANGI SALDO)
    $updateSppgQuery = "UPDATE sppg SET saldo = saldo - ? WHERE id = ?";
    $stmtUpdateSppg = $db->prepare($updateSppgQuery);
    $stmtUpdateSppg->execute([$data['nominal'], $data['sppg_id']]);
    
    $db->commit();
    
    echo json_encode(['status' => 'success', 'message' => 'Transfer berhasil', 'trx_id' => $trx_id]);
